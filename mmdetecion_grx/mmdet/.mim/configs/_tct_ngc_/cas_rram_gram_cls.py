...
model = dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='work_dir/roi_attention/cas_rram+gram/epoch_12.pth'
    ),
    backbone=dict(
        type='ResNet',
        depth=50,
...
ann_root = '../data/TCT_NGC/annotations/'
classes = (
    #'mesothelial_cell', 
    # 'blood_cell',
    # 'tissue_cell',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=ann_root + 'new_train.json',
    ...
    val=dict(
        type=dataset_type,
        ann_file=ann_root + 'new_val.json',
        img_prefix=data_root,
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=ann_root + 'new_test.json',
        img_prefix=data_root,
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
        ...
        classes=classes))
evaluation = dict(interval=6, metric='bbox',evaluate_per_class=True)
...
work_dir = 'work_dir/roi_attention/cas_rram+gram/with_cls_head/'
#gpu_ids = range(0, 1)
