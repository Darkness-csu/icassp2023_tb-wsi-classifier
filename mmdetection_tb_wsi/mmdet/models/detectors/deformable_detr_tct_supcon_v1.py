...
from ..roi_heads.deformable_cls_head_supcon_v1 import CLS_Head_SupCon_V1
from .single_stage import SingleStageDetector
#from .deformable_detr_tct import DeformableDETR_TCT
@DETECTORS.register_module()
class DeformableDETR_TCT_SupCon_V1(DETR):
    def __init__(self, *args, **kwargs):
        super(DETR, self).__init__(*args, **kwargs)

        cls_head=dict(
            loss_supcon = dict(type='SupConLoss', loss_weight=2.0),
            in_index=0,
            loss_weight=3,#0.3
            pos_list=[1, 2, 3, 4, 5]
        )
        self.cls_head = CLS_Head_SupCon_V1(
            loss_supcon = cls_head['loss_supcon'],
...
#         print(decoder_querys.size())
        ...
        if self.cls_head is not None:
            cls_losses = self.cls_head.forward_train(decoder_querys, gt_labels)
    ...
    #         #decoder_querys = decoder_querys.detach()
    #         out = self.cls_head.simple_test(decoder_querys)
    #         return out[:,-1] #for img cls_score  
    #     else:
    #         return super().simple_test(img, img_metas, proposals=proposals, rescale=rescale)
