...
from ..roi_heads.deformdetr_cls_head_v2 import CLS_Head_V2
from .single_stage import SingleStageDetector
#from .deformable_detr_tct import DeformableDETR_TCT
@DETECTORS.register_module()
class DeformableDETR_TCT_SMEAR_V2(DETR):
...
#         return losses
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        if self.test_cfg.get('test_feature', False):    
            ...
            #print(pre_decoder_querys.size()) torch.size([1,256,301])
            decoder_querys = pre_decoder_querys[:,:,0]
            ...
            if self.test_cfg.get('cls_result', False):
                return torch.cat((decoder_querys, cls_results),dim=1)
            else:
