...
class CLS_Head_V4(nn.Module): #包含小型的MLP Mixer
    def __init__(self,
                 in_channels,
                 num_fc=0,
                 in_index=-1,
...
#         x = x_in[self.in_index]
    ...
    def forward_test(self, x_in):
        #x = x[self.in_index]
        out_feature = self.mlp_mixer.forward_test_smear(x_in)
        out_logit = self.mlp_mixer(x_in)
        #x = x.view(x.size(0), -1)
        #x = self.fc(out)
